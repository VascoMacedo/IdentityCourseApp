...
namespace IdentityCourseApp.Services
{
    public class SendGridEmail : ISendGridEmail
    {
        private readonly ILogger<ISendGridEmail> logger;

        public AuthMessageSenderOptions Options { get; set; }

        public SendGridEmail(IOptions<AuthMessageSenderOptions> options, ILogger<ISendGridEmail> _logger)
        {
        ...
        private async Task Execute(string apiKey, string subject, string message, string toEmail)
        {
            ...
            {
                From = new EmailAddress("saruga19@sapo.pt"),
                Subject = subject,
